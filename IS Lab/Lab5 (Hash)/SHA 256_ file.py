...
def hash_file(input_file_path, output_file_path):
    """Read lines from the input file, compute SHA-256 hashes, and write to the output file."""
    try:
        with open(input_file_path, 'r') as input_file, open(output_file_path, 'w') as output_file:
            for line in input_file:
                # Strip whitespace and compute the SHA-256 hash
                line = line.strip()
                if line:  # Skip empty lines
                    sha256_hash = compute_sha256_hash(line)
                    # Write the original line and its hash to the output file
                    output_file.write(f"{line} -> {sha256_hash}\n")
        print(f"SHA-256 hashes written to '{output_file_path}' successfully.")
